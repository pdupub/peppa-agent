...
任何可记录、可持久化、会影响未来召回或行为的数据，都必须有人类可理解的含义。模型可以参与判断、抽取、联想和排序，但它输出到长期状态的结果必须转换为 tag、node、edge、source、document、skill 等可读结构。不可解释的向量距离、模型隐藏状态或其他中间表示不能成为记忆相关性的唯一依据，也不应作为长期记忆的核心数据保存。

这一约束的目的，是把记忆、推理和模型调用分开：记忆保存可审计的事实、关系、证据和做事方式；推理发生在运行时；模型只是参与整理和使用记忆的工具之一，而不是记忆格式本身。
...
Peppa 的长期信息不应全部塞进同一个结构。不同内容应进入不同层次：

```text
脑子里的记忆：轻量、可联想、适合召回的语义图
笔记本里的记忆：复杂、精确、内容多的文档或工程记录
会做事的记忆：反复出现、会影响未来行为的 skill
```

项目规则、技术决策、当前任务状态、事实纠正等内容，在没有进入长期记忆前，可以看作工程化记忆。这类内容通常不应该完整记在语义记忆图中，而应该记录到 Markdown 文档、设计文档、任务文档或其他外部化载体中。

skill 化记忆不是保存事实，而是保存做事方式。它适合承载已经被多次验证、以后遇到类似任务应该复用的流程、判断标准、项目习惯和具体示例。skill 可以由文档和记忆中的真实案例沉淀出来，但不应该把一次性的偏好或尚未稳定的想法过早固化成 skill。
...
如果某类记忆会反复影响后续行动，例如“开发 Peppa 记忆系统时先检查现有文档和代码，再判断是否需要修改整体设计”，更适合在稳定后沉淀成 skill。语义图只需要保留能召回这个 skill 的入口和证据来源。
...
上下文文本
  -> 分类
  -> 判断是否值得保留
  -> 判断保留位置：语义记忆 / 外部文档 / skill 候选 / 临时 trace / 不保留
  -> 如果进入语义记忆，提取 tags / nodes / edges / sources
  -> 如果进入外部文档，语义记忆只保留项目或主题索引入口
  -> 如果适合 skill 化，先记录候选、示例和来源，等模式稳定后再固化为 skill
```

这意味着 memory extraction 的输出不应只包含 `tags/nodes/edges`，还应该包含分类和去向判断。

当前实现的 tool schema 还没有单独的 `skill_suggestions` 字段。短期内可以先把 skill 候选作为文档建议、任务记录或后续人工整理事项处理，避免为了未闭环的流程过早扩大 schema。
...
下一步要验证 memory extraction schema
...
skill 化记忆用于承载可重复执行的工作方式。它和 document 的区别是：

- document 主要保存事实、设计、决策和状态
- skill 主要保存以后应该怎么做
...
skill 应尽量来自真实记忆和文档中的多个具体例子，而不是从一次对话里直接泛化。语义图可以通过 tag、node 和 source 帮助未来召回相关 skill，但 skill 本身应作为独立文件进入 `skills/` 并提交到 git。
...
当前阶段的调试重点是验证大模型是否能稳定、合理地完成以下任务：
...
在上述基础稳定之后，记忆过程还需要补齐两类能力：

- 精确外置层：让 document suggestions 真正进入文档维护流程，并在合适时形成 skill 候选
- 图谱维护：支持 node 合并、摘要更新、别名或 tag 合并，以及基础冲突和覆盖处理
...
下一步不应继续过度细化记忆本体。优先顺序是：补齐精确外置层和基础 node 操作，然后实现第一版 recall，让已保存的 tag、graph、source、document 和未来的 skill 能进入实际对话上下文。
